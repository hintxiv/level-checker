...
export async function fetchFight(reportID: string, fightID: number): Promise<Fight> {
    ...
    return {
        ...
        friends: friends,
        pets: friendlyPets,
        inProgress: reportFight.inProgress,
    }
}
...
export async function fetchDamageTables(fight: Fight): Promise<any> {
    const { start, end } = await fetchFight(fight.reportID, fight.fightID)
    const result = await fflogs.get(
        `tables/damage-done/${fight.reportID}`,
